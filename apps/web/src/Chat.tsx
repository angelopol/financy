...
import { MessageCircle, X, Send, Sparkles, Trash2, Type, LoaderCircle, ArrowUp, RefreshCw, Check, Ban } from 'lucide-react';
import Markdown from 'react-markdown';
import { api } from './api';
type Action={id:string;kind:string;summary:string;status:'pending'|'executing'|'confirmed'|'cancelled'};
type Message={id:string;role:'user'|'model';content:string;context_at?:string;request_id:string;actions:Action[]};
export function Chat(){
  ...
  async function resolveAction(actionId:string,decision:'confirm'|'cancel'){
    ...
    try{
      ...
      setMessages(prev=>prev.map(m=>m.actions.some(a=>a.id===actionId)?{...m,actions:m.actions.map(a=>a.id===actionId?{...a,status}:a)}:m));
  ...
  return <>
    ...
    <dialog ref={dialog} className="chat-dialog" aria-labelledby="chat-title" onCancel={e=>{e.preventDefault();setOpen(false);}} onClick={e=>{if(e.target===dialog.current)setOpen(false);}}>
      <div className="chat-header"><span className="chat-brand"><Sparkles size={23}/></span><div><h2 id="chat-title">Tu copiloto financiero</h2><p>Financy IA · Gemini 3.5 Flash-Lite</p></div><button className="icon-button" aria-label="Ajustar tamaño de letra del chat" onClick={()=>{setOpen(false);navigate('/profile');}}><Type size={18}/></button><button className="icon-button" aria-label="Borrar conversación y memoria" disabled={busy||loading} onClick={()=>setConfirmClear(true)}><Trash2 size={18}/></button><button className="icon-button" aria-label="Cerrar chat" onClick={()=>setOpen(false)}><X size={20}/></button></div>
      ...
      <div className="chat-scroll" aria-label="Conversación" role="log" aria-live="polite" aria-relevant="additions">
        ...
        {messages.map(m=><article key={m.id} className={'chat-message '+m.role} aria-label={m.role==='user'?'Tú':'Financy IA'}><span className="chat-speaker">{m.role==='user'?'Tú':'Financy IA'}</span><div className="chat-markdown"><Markdown skipHtml components={{a:({href,children})=>href&&href.startsWith('/')?<Link to={href} onClick={()=>setOpen(false)}>{children}</Link>:<span>{children}</span>,img:()=>null}}>{m.content}</Markdown></div>{m.actions.map(action=><div key={action.id} className={'chat-action '+action.status}>
          <p>{action.summary}</p>
          {(action.status==='pending'||action.status==='executing')&&<div>
            <button className="small-button" disabled={resolving===action.id} onClick={()=>void resolveAction(action.id,'confirm')}><Check size={14}/> Confirmar</button>
            <button className="small-button" disabled={resolving===action.id} onClick={()=>void resolveAction(action.id,'cancel')}><Ban size={14}/> Cancelar</button>
          </div>}
          {action.status==='confirmed'&&<span className="chat-action-status"><Check size={14}/> Aplicado</span>}
          {action.status==='cancelled'&&<span className="chat-action-status"><Ban size={14}/> Cancelado</span>}
