...
import { MessageCircle, X, Send, Sparkles, Trash2, LoaderCircle, ArrowUp, RefreshCw } from 'lucide-react';
import Markdown from 'react-markdown';
import { api } from './api';
type Message={id:string;role:'user'|'model';content:string;context_at?:string;request_id:string};
  ...
  return <>
    ...
    <dialog ref={dialog} className="chat-dialog" aria-labelledby="chat-title" onCancel={e=>{e.preventDefault();setOpen(false);}} onClick={e=>{if(e.target===dialog.current)setOpen(false);}}>
      ...
      <div className="chat-scroll" aria-label="Conversación" role="log" aria-live="polite" aria-relevant="additions">
        ...
        {messages.map(m=><article key={m.id} className={'chat-message '+m.role} aria-label={m.role==='user'?'Tú':'Financy IA'}><span className="chat-speaker">{m.role==='user'?'Tú':'Financy IA'}</span><div className="chat-markdown"><Markdown skipHtml components={{a:({children})=><span>{children}</span>,img:()=>null}}>{m.content}</Markdown></div>{m.role==='model'&&m.context_at&&<small>Datos consultados: {new Intl.DateTimeFormat('es-VE',{dateStyle:'short',timeStyle:'short',timeZone:'America/Caracas'}).format(new Date(m.context_at))}</small>}</article>)}
      ...
      <div className="chat-footer">Solo lectura · La consulta y los datos relevantes se envían a Gemini.<br/>Memoria privada de tu cuenta. Puedes borrarla cuando quieras.</div>
    </dialog>
  </>;
}
