...
export default function CreateEarningForm({ projectId = null }) {
    ...
    const descriptionChanged = (value) => {
        setData((current) => ({...current, description: value, slug: slugTouched ? current.slug : descriptionTags(value)}));
    };
    const submit = (event) => {
        event.preventDefault();
        post(route('earnings.store'), { onSuccess: () => { reset(); setSlugTouched(false); } });
    };
    ...
    return (
        <form onSubmit={submit} className="mt-6 space-y-6">
            ...
            <div><InputLabel htmlFor="earning-slug" value="Keywords" /><TagInput id="earning-slug" value={data.slug} onChange={(tags) => { setSlugTouched(true); setData('slug', tags); }} /><InputError message={errors.slug} className="mt-2" /></div>
